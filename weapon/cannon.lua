...
function UpdateTarget()
    ...
    if newTarget ~= nil and target ~= nil and newTarget.Id == target.Id then
        targetAcceleration = newTarget.Velocity - target.Velocity
...
function ComputeAim(weaponPosition, weaponSpeed)
    ...
    I:LogToHud(string.format("Lead time: %0.2f s", t))
    return aim, t
end
...
function ComputeFlightTime(aim, weaponSpeed)
    ...
    if discriminant > 0 then
        ...
        return (lower >= 0 and math.sqrt(lower)) or (upper >= 0 and math.sqrt(upper)) or nil
    else
        return nil
    end
end

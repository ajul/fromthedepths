...
def wedgeForm(x):
    ...
    for i, (slope, forwardCD) in enumerate(shapes):
        ...
        volume += 1/2 * abs(currentRadius + nextRadius) * length
        currentRadius = nextRadius
...
initial = numpy.ones((len(shapes) + 1,)) * 0.1
bounds = [(0, None)] * (len(shapes) + 1)

result = scipy.optimize.minimize(computeScore(pyramidalForm), initial, bounds = bounds)
print(result.x, pyramidalForm(result.x), result.fun)

result = scipy.optimize.minimize(computeScore(wedgeForm), initial, bounds = bounds)
